...
export const VehicleEntry = (props) => {
  ...
  const submit = (e) => {
    ...
    for (let i = 0; i < vehicles.length; i++) {
      ...
      if (
        vehicle.vehicleNumber === formValues.vehicleNumber &&
        vehicle.vehicleType === formValues.vehicleType &&
        t < 1
      ) {
        modified = true;
        const tolls = JSON.parse(localStorage.getItem("tolls"));
        const returntariff =
          tolls[formValues.toll][formValues.vehicleType]["return"];
        newValue = {
          ...formValues,
      ...
      <form className="body" onSubmit={submit}>
        <div>
          ...
          <input
            ...
            required
            onChange={(e) =>
              setFormValues({ ...formValues, vehicleNumber: e.target.value })
            }
          />
