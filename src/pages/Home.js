...
export const Home = () => {
  ...
  const [tollNames, setTollNames] = useState([
    ...new Set(tableValues.map((t) => t.toll)),
  ]); // filter icon values
  ...
  const changeData = (search) => {
    ...
    const newData = tableValues.filter((t) =>
  ...
  return (
    <>
      <main className="header">
        ...
        <Filter width={20} height={20} onClick={showFilters} />
        {modals.filter && (
          <ul>
            <li onClick={() => showToll("")}>All</li>
            {tollNames.map((toll, idx) => (
              <li key={idx} onClick={() => showToll(toll)}>
                {toll}
              </li>
            ))}
          </ul>
      ...
      <table>
        ...
        <tbody>
          {tableValues.map((tableValue, idx) => {
            const { toll, vehicleType, vehicleNumber, tariff, timestamp } =
              tableValue;
      ...
      {modals.vehicle && (
        <VehicleEntry setModals={setModals} setTableValues={setTableValues} />
      )}
      {modals.toll && <TollEntry setModals={setModals} />}
    </>
  );
};
